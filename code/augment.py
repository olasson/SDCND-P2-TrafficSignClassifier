...
# Padding used to preserve image dimensions
...
def random_transforms(image, mask = None):
    ...
    if mask[0] == 1:
        scale_fx = np.random.uniform(0.75, 1.25)
        scale_fy = np.random.uniform(0.75, 1.25)        
        image = scale_image(image, scale_fx, scale_fy)

    if mask[1] == 1:
        T_x = np.random.randint(-4, 4) 
        T_y = np.random.randint(-4, 4)
        image = translate_image(image, T_x, T_y)

    if mask[2] == 1:
        border_offset = np.random.uniform(-3, 3)
